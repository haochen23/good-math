...
export const VerticalCalculation: React.FC<VerticalCalculationProps> = ({
  ...
  useEffect(() => {
    ...
    const runAnimation = async () => {
      // Initial delay
      await new Promise(r => { timeoutId = setTimeout(r, baseTiming); });
      ...
      for (let col = 0; col < maxDigits; col++) {
        ...
        // Calculate phase
        setPhase('calc');
        await new Promise(r => { timeoutId = setTimeout(r, baseTiming * 1.5); });
        ...
        if (hasCarryOrBorrow) {
          setPhase('carry');
          await new Promise(r => { timeoutId = setTimeout(r, baseTiming * 1.2); });
        }
        
        // Write phase
        setPhase('write');
        setWrittenDigits(prev => [...prev, (column as ColumnCalc).resultDigit ?? (column as SubtractionColumnCalc).resultDigit]);
        await new Promise(r => { timeoutId = setTimeout(r, baseTiming * 0.8); });
      }
  ...
  }, [problem.id, animationSpeed, maxDigits, columns, onComplete, problem.operation]);
    ...
    <div className={`flex flex-col items-center gap-4 ${className}`}>
      ...
      <div className="vertical-calc relative bg-white rounded-3xl shadow-medium p-4 md:p-6">
        ...
        >
          {[...op1Digits].reverse().map((digit, i) => {
            ...
            return (
              <motion.span 
                key={`op1-${i}`} 
                className={`${digitWidth} text-center rounded-lg transition-all duration-300 ${
                  isHighlighted ? 'bg-yellow-200 text-yellow-800 scale-110' : ''
                }`}
              >
                {digit !== null ? digit : ''}
              </motion.span>
        ...
        <motion.div
          ...
          animate={{ opacity: 1 }}
        >
          <span className={`${digitWidth} text-center ${
            problem.operation === 'addition' ? 'text-secondary-500' : 'text-accent-500'
          }`}>
            {operationSymbol}
          </span>
          {[...op2Digits].reverse().map((digit, i) => {
            ...
            return (
              <motion.span 
                key={`op2-${i}`} 
                className={`${digitWidth} text-center text-gray-800 rounded-lg transition-all duration-300 ${
                  isHighlighted ? 'bg-yellow-200 text-yellow-800 scale-110' : ''
                }`}
              >
                {digit !== null ? digit : ''}
          ...
          >
            <motion.div
              className={`text-xl md:text-2xl font-bold ${
                isCorrect ? 'text-secondary-500' : 'text-red-500'
              }`}
